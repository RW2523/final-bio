...
class FCN(nn.Module):
    def __init__(self, n_channels, n_classes, out_channels=128, backbone=True, len_sw=128):
        ...
        def _conv_pool_out_len(seq_len):
            out = seq_len
            for _ in range(3):
                # conv1d: kernel=8, stride=1, padding=4
                out = out + 1
                # pool1d: kernel=2, stride=2, padding=1
                out = (out // 2) + 1
            return out

        self.out_len = _conv_pool_out_len(len_sw)
...
class DeepConvLSTM(nn.Module):
    def __init__(self, n_channels, n_classes, conv_kernels=64, kernel_size=5, LSTM_units=128, backbone=True):
        ...
        self.conv1 = nn.Conv2d(1, conv_kernels, (kernel_size, 1), padding=(2, 0))
        self.conv2 = nn.Conv2d(conv_kernels, conv_kernels, (kernel_size, 1), padding=(2, 0))
        self.conv3 = nn.Conv2d(conv_kernels, conv_kernels, (kernel_size, 1), padding=(2, 0))
        self.conv4 = nn.Conv2d(conv_kernels, conv_kernels, (kernel_size, 1), padding=(2, 0))
...
class Classifier(nn.Module):
    def __init__(self, bb_dim, n_classes, head_type='linear', hidden_dim=512, hidden_dim2=256, dropout=0.2):
        ...
        if head_type == 'linear':
            ...
            raise ValueError(f'Unsupported classifier head_type={head_type!r}')
